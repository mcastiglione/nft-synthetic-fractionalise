...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
	 ...
	 */
	constructor(string memory name_, string memory symbol_, address vaultAddress, address governanceAddress) {
		_name = name_;
		_symbol = symbol_;
	...
	// Allows the caller to bid for a given NFT
	function bid(uint256 nftId) public payable {
		bidStruct memory mybid = bidStruct(nftId, msg.sender, msg.value, block.timestamp, false);
		_bidCounter.increment();
		bids[_bidCounter.current()] = mybid;

		emit eventBid(_bidCounter.current(), msg.sender, msg.value, block.timestamp);
	}

	// Accept a bid for a given NFT
	function acceptBid(uint256 bidId) public {
		uint256 nftId = bids[bidId].nftId;
		address from = bids[bidId].from;
		uint256 amount = bids[bidId].amount;
		uint256 timestamp = bids[bidId].timestamp;
		bool accepted = bids[bidId].accepted;

		address nftOwner = ownerOf(nftId);
		require(nftOwner == msg.sender, "You are not the owner of the NFT!");

		// bids last for 4 hours
		uint bidOfferTimeLimit = governanceData.bidOfferTimeLimit();
		require(block.timestamp - timestamp <= bidOfferTimeLimit, "Bid is over!");
		
		// can't accept a bid twice
		require(accepted == false, "Bid already accepted!");

		transferFrom(msg.sender, from, nftId);

	}

	// Withdraw money after 4 hours
	function withdrawBid(uint256 bidId) public {
		...
		require(block.timestamp - timestamp <= bidOfferTimeLimit, "Bid is over!");
		require(accepted == false, "Bid already accepted!");
		require(from == msg.sender, "Bid not made by same address!");
