...
contract NFTFractions is IERC20, Pausable, Ownable {
	...
	constructor(address beneficiary,
		...
		uint initialMintAmount = totalSupply_ * 10 ** decimals_;
		uint sellAmount = _supplyToBeIssued * 10 ** decimals_;
	...
	function buy() payable public {
		...
		uint _fee = governanceData.tradingFee();
		uint fee = msg.value / 100 * _fee;
		uint raisedFunds_ = msg.value - fee;
	...
	*/
	...
	function UniswapInitialPrice() public view returns (uint256) {
		uint _remainingSupply = remainingSupply();
		uint _UniSwapInitialPrice = _remainingSupply / (supplyToBeIssued * _priceInWei);
