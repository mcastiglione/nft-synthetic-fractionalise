        ...
        for (const fnName of INTERFACES[k]) {
          ...
          describe(fnName, function () {
            it('has to be implemented', function () {
              let functions = Object.keys(this.contractUnderTest.interface.functions);

              expect(functions.filter((fn) => web3.eth.abi.encodeFunctionSignature(fn) === fnSig).length).to.equal(1);
            });
          });
        }
      });
